...
function loadSeal() {
  document.getElementById("provider").value = localStorage.getItem(KEYS.provider) || "gemini";
...
async function composeViaGemini(key, model, images) {
  const parts = [
    {
      text: "你是中文理科作业排版员。把照片中的手写解题过程整理成 JSON。solution 的 kind 只能是 text、math、rowops。矩阵用 pmatrix。只输出 JSON。",
    },
  ];
...
async function compose() {
  ...
  try {
    ...
    const draft = await composeViaGemini(
      key,
