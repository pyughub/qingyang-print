...
只输出一个 JSON 对象，不要 markdown。latex 写真正的公式，例如 \\begin{pmatrix} 1 & 0 \\\\ 0 & 1 \\end{pmatrix}。任意符号必须写成 \\forall，不要写成 forall 或 for all。JSON 会自己转义，不要再把反斜杠加倍。同一矩阵不要重复写两遍。

字段：
- title 可用「第 n 题」，仅作文件名，不要印在稿面上
- problems[].number / stem / solution
- solution[].kind 只能是 text、math、rowops
- text：中文说明，行内公式必须写成配对的 $...$，禁止只写开头的 $$
- math.latex：不含 $$ 的独立公式，矩阵用 pmatrix
...
function mixedToFragment(text) {
  ...
  let last = 0;
  let m;
  const src = repairMathText(String(text || ""));
  while ((m = re.exec(src))) {
...
function toTex(draft) {
  ...
  const who = [name, sid].filter(Boolean).join("\\quad");
